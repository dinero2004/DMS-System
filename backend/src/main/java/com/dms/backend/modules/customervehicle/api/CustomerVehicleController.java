...
public class CustomerVehicleController {
    ...
    private final CarImageStorageService carImageStorage;
    ...
    public CustomerVehicleController(ClientRepository cr, CarRepository car, WorkshopJobRepository jr,
            ...
            SalesContractRepository scr, FinancingOfferRepository fr, CarImageStorageService carImageStorage) {
        ...
        this.carImageStorage = carImageStorage;
    }
    ...
    public ApiResponse deleteClient(@PathVariable String id) {
        ...
        for (CarEntity car : carRepository.findByClientId(id)) {
            try { carImageStorage.deleteAllForCar(car); } catch (IOException e) {
                throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to delete car images");
            }
    ...
    public ApiResponse registerCar(@RequestBody @Valid RegisterCarRequest request) {
        ...
        car.setStammnummer(blankToNull(request.stammnummer()));
        car.setTypeApproval(blankToNull(request.typeApproval()));
        car.setVehicleRole(role);
    ...
    public ApiResponse updateCar(@PathVariable String id, @RequestBody @Valid RegisterCarRequest request) {
        ...
        if (request.typeApproval() != null) car.setTypeApproval(blankToNull(request.typeApproval()));
        ...
        carRepository.save(car);
        try {
            carImageStorage.relocateIfNeeded(car, oldMake, oldModel);
        } catch (IOException e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to relocate car images");
        }
    ...
    public ApiResponse deleteCar(@PathVariable String id) {
        ...
        try {
            carImageStorage.deleteAllForCar(car);
        } catch (IOException e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to delete car images");
        }
    ...
    public record RegisterCarRequest(String clientId, String branchId, String make,
        @NotBlank String model, String plate, String vin, String stammnummer, String typeApproval, String vehicleRole,
