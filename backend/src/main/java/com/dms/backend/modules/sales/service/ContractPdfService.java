...
public class ContractPdfService {
    ...
    public ContractPdfService(MessageSource messages) {
    ...
    public byte[] render(SalesContractEntity ct, ClientEntity client, CarEntity car, Locale locale) {
        ...
        try {
            ...
            if (client.getPhone() != null) addr.add(new Chunk(m(loc, "pdf.word.tel") + " " + client.getPhone() + "\n", BODY));
            if (client.getEmail() != null) addr.add(new Chunk(client.getEmail() + "\n", BODY));
            if (client.getBirthday() != null) addr.add(new Chunk(m(loc, "pdf.contract.dob", DF.format(client.getBirthday())), BODY));
            doc.add(addr);
            doc.add(sp(12));

            doc.add(new Paragraph(m(loc, "pdf.contract.title"), TITLE));
            doc.add(sp(6));

            PdfPTable dateRow = new PdfPTable(4);
            dateRow.setWidthPercentage(100);
            dateRow.setWidths(new float[]{25, 25, 25, 25});
            addHeaderValue(dateRow, m(loc, "pdf.contract.contractDate"), DF.format(ct.getContractDate()));
            addHeaderValue(dateRow, m(loc, "pdf.contract.customerNo"), client.getId().substring(0, 6));
            addHeaderValue(dateRow, m(loc, "pdf.contract.phone"), client.getPhone() != null ? client.getPhone() : dash(loc));
            addHeaderValue(dateRow, m(loc, "pdf.contract.insurance"), ct.getInsuranceCompany() != null ? ct.getInsuranceCompany() : dash(loc));
            doc.add(dateRow);
            doc.add(sp(6));
            ...
            if (car != null) {
                ...
                addHeaderValue(vInfo, m(loc, "pdf.contract.plate"), ct.getRegistrationPlate() != null ? ct.getRegistrationPlate() : (car.getPlate() != null ? car.getPlate() : dash(loc)));
                addHeaderValue(vInfo, m(loc, "pdf.contract.firstReg"), car.getFirstRegistrationDate() != null ? DF.format(car.getFirstRegistrationDate()) : dash(loc));
                addHeaderValue(vInfo, m(loc, "pdf.contract.chassis"), car.getVin() != null ? car.getVin() : dash(loc));
                addHeaderValue(vInfo, m(loc, "pdf.contract.mileage"), car.getMileageKm() != null ? String.format("%,d", car.getMileageKm()).replace(",", "'") + " km" : dash(loc));
                addHeaderValue(vInfo, m(loc, "pdf.contract.fuelType"), car.getFuelType() != null ? car.getFuelType() : dash(loc));
                addHeaderValue(vInfo, m(loc, "pdf.contract.year"), car.getModelYear() != null ? String.valueOf(car.getModelYear()) : dash(loc));
                doc.add(vInfo);
            }
            doc.add(sp(10));
            ...
            String[] headers = {m(loc, "pdf.contract.pos"), m(loc, "pdf.contract.description"), m(loc, "pdf.contract.qty"), m(loc, "pdf.contract.amountChf")};
            ...
            String vehiclePart = car != null ? ((car.getMake() != null ? car.getMake() + " " : "") + car.getModel()) : "—";
            addContractRow(itemTable, String.valueOf(pos++), m(loc, "pdf.contract.vehicleLine", vehiclePart), "1", fmtChf(ct.getSellingPriceCents()));

            long prepFee = ct.getPrepFeeCents() != null ? ct.getPrepFeeCents() : 0;
            if (prepFee > 0) {
                subtotalCents += prepFee;
                addContractRow(itemTable, String.valueOf(pos++), m(loc, "pdf.contract.prepFee"), "1", fmtChf(prepFee));
            }
            ...
            addTotalRow(totals, m(loc, "pdf.contract.subtotalExcl"), fmtBd(exclVat), false);
            addTotalRow(totals, m(loc, "pdf.contract.vat"), fmtBd(vatAmt), false);
            addTotalRow(totals, m(loc, "pdf.contract.totalIncl"), "CHF " + fmtBd(subtotal), true);
            doc.add(totals);

            if (ct.getNotes() != null && !ct.getNotes().isBlank()) {
                doc.add(sp(6)); doc.add(new Paragraph(m(loc, "pdf.contract.notes"), H2)); doc.add(new Paragraph(ct.getNotes(), BODY));
            }

            doc.add(sp(20));
            PdfPTable sig = new PdfPTable(2); sig.setWidthPercentage(100);
            sig.addCell(sigCell(m(loc, "pdf.contract.seller", CO)));
            sig.addCell(sigCell(m(loc, "pdf.contract.buyer", clientName)));
            doc.add(sig); doc.add(sp(8));
            doc.add(new Paragraph(m(loc, "pdf.contract.placeDate", DF.format(ct.getContractDate())), BODY));
            doc.add(sp(10));
            doc.add(new Paragraph(CO + " · " + ADDR + " · " + m(loc, "pdf.word.tel") + " " + TEL, SMALL));
