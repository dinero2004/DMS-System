...
import HomeSearch, { type HomeSearchSelect } from './HomeSearch.tsx'
import SalesCalendar from './SalesCalendar.tsx'
import CarEditMask from './CarEditMask.tsx'
...
type Page = 'home' | 'service' | 'sales' | 'calendar' | 'clients' | 'finance' | 'admin'
...
function App() {
  ...
  const [serviceView, setServiceView] = useState<'board' | 'create'>('board')
  const [carEditCarId, setCarEditCarId] = useState<string | null>(null)
  const [carEditStep, setCarEditStep] = useState<'data' | 'images'>('data')
  ...
  const carEditCar = useMemo(() => (carEditCarId ? cars.find(c => c.id === carEditCarId) ?? null : null), [cars, carEditCarId])
  ...
  function openCarEdit(carId: string) {
    setCarEditCarId(carId)
    setCarEditStep('data')
  }

  function closeCarEdit() {
    setCarEditCarId(null)
    setCarEditStep('data')
  }

  async function saveCarEdit(carId: string, payload: Record<string, unknown>) {
    if (payload.model && String(payload.model).trim() === '') return setMsg(t('msg.modelRequired'))
    const vin = payload.vin as string | undefined
    if (vin?.trim() && vin.trim().length !== 17) return setMsg(t('msg.vin17'))
    setLoading(true)
    try {
      await put<ApiResponse>(`/api/v1/cars/${carId}`, payload)
      await refresh()
      setMsg(t('msg.inventoryUpdated'))
    } catch (e) {
      setMsg(e instanceof Error ? e.message : t('msg.error'))
      throw e
    } finally {
      setLoading(false)
    }
  ...
  async function createJob() {
    ...
    try {
      ...
      setJobTitle(''); setJobDesc(''); setJobItems([]); setServiceView('board'); setLastCreatedJobId(res.id); await refresh(); setMsg(t('msg.jobCreated'))
    } catch (e) { setMsg(e instanceof Error ? e.message : t('msg.error')) } finally { setLoading(false) }
  }
  ...
  async function previewPdf(url: string) {
    try {
      const raw = (i18n.resolvedLanguage ?? 'en').split('-')[0]
      const lang = ['de', 'en', 'fr', 'it'].includes(raw) ? raw : 'en'
      const sep = url.includes('?') ? '&' : '?'
      const r = await fetch(`${API}${url}${sep}lang=${lang}`, { credentials: 'include' })
      if (r.status === 401) {
        onSessionExpired()
        return
      }
  ...
  return (
    <div className="layout">
      <aside className="sidebar">
        ...
        <SBtn id="calendar" label={t('nav.calendar')} />
      ...
      <main className="content">
        ...
        {carEditCar ? (
          <CarEditMask
            car={carEditCar}
            step={carEditStep}
            onStepChange={setCarEditStep}
            onClose={closeCarEdit}
            onSave={payload => saveCarEdit(carEditCar.id, payload)}
            apiBase={API}
            loading={loading}
          />
        ) : (<>
        ...
        {page === 'home' && (
          <>
            <HomeSearch
              clients={clients}
              cars={cars}
              dealerStockClientId={DEALER_STOCK_CLIENT_ID}
              onSelect={(sel: HomeSearchSelect) => {
                if (sel.type === 'client') {
                  setPage('clients')
                  setClientsView('database')
                  setDbSearch(clients.find(c => c.id === sel.clientId)?.displayName ?? '')
                  setExpandedClientId(sel.clientId)
                } else {
                  const owner = clients.find(c => c.id === sel.clientId)
                  const car = cars.find(c => c.id === sel.carId)
                  setPage('clients')
                  setClientsView('database')
                  setDbSearch(car ? `${car.make ?? ''} ${car.model}`.trim() : owner?.displayName ?? '')
                  setExpandedClientId(sel.clientId)
                }
              }}
            />
            <Dashboard clients={clients} cars={cars} jobs={jobs} leads={leads} invoices={invoices} />
          </>
        )}
        ...
        {page === 'service' && (
          <section>
            <div className="section-toggle">
              ...
              <button type="button" className={serviceView === 'create' ? 'active' : ''} onClick={() => setServiceView('create')}>{t('service.createWorkshopJob')}</button>
            </div>
              {serviceView === 'create' && (
              ...
              {serviceView === 'create' && lastCreatedJobId && <div className="quick-action"><span>{t('service.jobCreatedOk')}</span><button type="button" onClick={() => createInvoiceFromJob(lastCreatedJobId)}>{t('service.createInvoice')}</button><button type="button" className="mini" onClick={() => setLastCreatedJobId(null)}>{t('common.dismiss')}</button></div>}
              {serviceView === 'board' && (
        ...
        {page === 'calendar' && (
          <section className="calendar-page-section">
            <SalesCalendar fullPage jobs={jobs} leads={leads} contracts={contracts} invoices={invoices} inventoryCars={inventoryCars} />
          </section>
        )}
                      ...
                      return (<>
                        ...
                        {isExpanded && (<tr key={c.id + '-detail'}><td colSpan={8}>
                          <div className="db-detail">
                            ...
                            {clientCars.length > 0 && <><h5>{t('common.expandVehicles', { n: clientCars.length })}</h5><ul className="db-sub-list">{clientCars.map(car => (<li key={car.id}><strong>{car.make ? `${car.make} ` : ''}{car.model}</strong>{car.plate ? ` (${car.plate})` : ''}{car.fuelType ? ` · ${fuelTypeLabel(car.fuelType)}` : ''}{car.vin ? ` · ${t('common.vin')} ${car.vin}` : ''}{car.modelYear ? ` · ${car.modelYear}` : ''}{car.firstRegistrationDate ? ` · ${car.firstRegistrationDate}` : ''} <button type="button" className="mini" onClick={() => openCarEdit(car.id)}>{t('common.editLabel')}</button> <button type="button" className="mini" style={{ background: '#fee2e2', color: '#dc2626' }} onClick={() => setConfirmDelete({ type: 'car', id: car.id, label: `${car.make ?? ''} ${car.model}` })}>{t('common.delete')}</button></li>))}</ul></>}
